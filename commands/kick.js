...
module.exports.run = async (bot, message, args) => {
  ...
  //%kick @daeshan para kickar a pessoa
     if(!message.member.hasPermission("MANAGE_MESSAGES")) return message.channel.send("Você não pode fazer isso!").then(msg => msg.delete(5000));
     if (args[0] == "help"){
      message.reply("use: %kick <user> <motivo>");
      return;
    }
      let kUser = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[0]));
      if(!kUser) return message.channel.send("Não consigo o encontrar usuário!").then(msg => msg.delete(5000));
      let kReason = args.join(" ").slice(22);

      if(kUser.hasPermission("MANAGE_MESSAGES")) return message.channel.send("Essa pessoa não pode ser kickada!").then(msg => msg.delete(5000));
