...
module.exports.run = async (bot, message, args) => {
  ...
  if(!rMember) return message.reply("Não foi possível encontrar o usuário.").then(msg => msg.delete(5000));
  let role = args.join(" ").slice(22);
  if(!role) return message.reply("Especifique o cargo!").then(msg => msg.delete(5000));
  let gRole = message.guild.roles.find(`name`, role);
  if(!gRole) return message.reply("Não foi possível encontrar esse Cargo.").then(msg => msg.delete(5000));

  if(!rMember.roles.has(gRole.id)) return message.reply("Não existe nenhum cargo com esse nome.").then(msg => msg.delete(5000));
